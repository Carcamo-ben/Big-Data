...
"""
All the definitions here are to properly read h5 files from MSD
"""
...
def get_all_titles(basedir,ext='.h5') :
    ...
    for root, dirs, files in os.walk(basedir):
        ...
        for f in files:
            ...
            titles.append(get_title(h5))
            h5.close()
    return titles
    
f=sys.argv[1]
h5 = open_h5_file_read(f)
time_lapse_bars=np.average(np.diff(get_beats_start(h5)))*get_time_signature(h5)
seconds_preview=10
end_preview=int(seconds_preview/time_lapse_bars)
pointa=get_bars_start(h5)[int(get_duration(h5)/len(get_sections_start(h5)))]
pointb=get_bars_start(h5)[int((get_duration(h5)/len(get_sections_start(h5))))+int(end_preview)]
print (get_artist_name(h5),[get_title(h5),pointa,pointb])
